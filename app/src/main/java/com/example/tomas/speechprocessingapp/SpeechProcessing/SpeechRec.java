...
public class SpeechRec extends Thread{
    ...
    public SpeechRec(Context context,String pathdata, String id, String task)
    {
    ...
    private void startSpeechRecording() {
            ...
            String format = simpleDateFormat.format(date);
            File filePcm = new File(pathData + File.separator +id+"_" + format + ".pcm");
            File fileWav = new File(pathData + File.separator + "WAV" + File.separator +id+"_" +  format + ".wav");
            File fileWav2 = new File(pathData+File.separator+"SPEECHTASKS"+File.separator+task+ File.separator +id+"_" +  format + ".wav");
            //dataouputStream is the audio signal converted to bytes
            pathNew=pathData+File.separator+"SPEECHTASKS"+File.separator+task+ File.separator +id+"_" +  format + ".wav";
        try {
            ...
            wavFileWriter.start();
            WavFileWriter wavFileWriter2 = new WavFileWriter(SAMPLE_RATE_HZ, filePcm, fileWav2,context, format);
            wavFileWriter2.start();
